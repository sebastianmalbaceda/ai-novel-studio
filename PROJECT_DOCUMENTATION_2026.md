...
### `.agents/` — Directorio Unificado Estándar (2026)

`.agents/` es el directorio estándar unificado que centraliza skills, reglas,
configuración y recursos del agente en un único punto de referencia que
cualquier herramienta compatible puede leer. Reemplaza la fragmentación
de carpetas específicas de cada herramienta (`.claude/`, `.opencode/`,
`.cursor/`, `.windsurf/`, `.codex/`, etc.).

**Propósito:** Eliminar la duplicación y fragmentación de conocimiento
entre herramientas. Un solo directorio que todos los agentes entienden,
sin importar si se ejecuta en Claude Code, OpenAI Codex, Cursor, OpenCode,
Windsurf, o cualquier otra herramienta compatible.

**Relación con `AGENTS.md`:**
- `AGENTS.md` = instrucciones de comportamiento del agente (reglas del proyecto)
- `.agents/` = contenedor de capacidades, skills, configuraciones y recursos

**Estructura del directorio `.agents/`:**

```
.agents/
├── skills/                    ← Skills compartidos (estándar)
│   ├── generate-tests/
│   │   ├── SKILL.md
│   │   └── scripts/
│   │       └── run-tests.sh
...
│   └── code-review/
│       └── SKILL.md
├── rules/                     ← Reglas específicas por contexto o tipo de archivo
│   ├── typescript.md          ← Reglas para archivos TypeScript
│   ├── database.md            ← Reglas para migraciones y queries
│   └── security.md            ← Reglas de seguridad obligatorias
├── config.json                ← Configuración unificada del agente
├── mcp.json                   ← Configuración MCP centralizada
├── prompts/                   ← Prompts reutilizables (complementa PROMPTS.md)
│   ├── code-review.md
│   └── generate-docs.md
└── memory/                    ← Memoria persistente del agente entre sesiones
    └── project-context.md
```
...
```json
{
  "agent": {
    ...
    "fallbackModels": ["glm-5", "qwen-max"],
    "maxContextTokens": 128000,
    "toolPermissions": ["file_read", "file_write", "bash", "git"],
    "restrictedDirectories": ["node_modules/", ".git/", "dist/"],
    "autoApprove": ["file_read", "git_status"],
    "requireConfirmation": ["file_write", "bash", "git_commit"]
  },
  "project": {
    "language": "typescript",
    "framework": "nextjs",
...
**Migración desde directorios específicos:**

| Directorio legacy        | Equivalente en `.agents/`         |
| ------------------------ | --------------------------------- |
...
| `.windsurfrules`         | `.agents/rules/windsurf.md`       |
| `.github/agents/`        | `.agents/config.json`             |
| `.opencode`              | `.agents/config.json`             |
| `~/.claude/skills/`      | `.agents/skills/` (commit al repo)|

**Compatibilidad con herramientas existentes:**

La mayoría de herramientas modernas leen `.agents/` como fuente primaria,
manteniendo compatibilidad con sus directorios legacy como fallback:

- **Claude Code:** Lee `.agents/skills/` como fuente principal de skills.
  Mantiene compatibilidad con `.claude/skills/` como fallback.
- **OpenAI Codex:** Lee `.agents/skills/` y `.agents/config.json`.
  Mantiene compatibilidad con `.codex/skills/` como fallback.
- **Cursor:** Lee `.agents/rules/` como reglas del proyecto.
  Mantiene compatibilidad con `.cursor/rules/` como fallback.
- **OpenCode:** Lee `.agents/config.json` para configuración de runtime.
  Mantiene compatibilidad con `.opencode` como fallback.
- **Windsurf:** Lee `.agents/rules/` y `.agents/skills/`.
  Mantiene compatibilidad con `.windsurfrules` como fallback.
- **GitHub Copilot:** Lee `.agents/` para reglas y skills del repositorio.
  Mantiene compatibilidad con `.github/copilot-instructions.md`.

> `.agents/` tiene prioridad sobre los directorios legacy cuando ambos existen.

---

### `CLAUDE.md` — Claude específico (legacy)

Context and instruction file specifically used by Claude-based tools
(Claude Code, Claude API agents). When both `AGENTS.md` and `CLAUDE.md`
exist, `AGENTS.md` takes precedence for cross-tool rules, and `CLAUDE.md`
handles Claude-specific nuances or overrides.

**Reemplazado por:** `.agents/rules/claude.md` para comportamientos
específicos de Claude dentro del estándar unificado.

---

### `.cursor/rules` — Cursor IDE (legacy)

Project-specific rules for the Cursor IDE AI assistant. Cursor recommends
the `.cursor/rules/` directory structure over the legacy `.cursorrules`
file at root.

**Reemplazado por:** `.agents/rules/` para reglas compartidas entre herramientas.
Mantener `.cursor/rules/` solo para comportamientos exclusivos de Cursor UI.

---

### `.windsurfrules` — Windsurf IDE (legacy)
...
**Reemplazado por:** `.agents/rules/windsurf.md`.

---

### `.github/copilot-instructions.md` — GitHub Copilot (legacy)
...
**Reemplazado por:** `.agents/rules/github.md` para comportamientos
específicos de GitHub dentro del estándar unificado.
...
**Reemplazado por:** `.agents/config.json`.
...
**Primary location:** `.agents/mcp.json` (unified standard)

**Legacy locations (still supported as fallbacks):**
...
```
# Primary standard (unified — all tools)
...
.codex/skills/                ← OpenAI Codex fallback
~/.codex/skills/              ← user-level

.github/agents/               ← GitHub Copilot fallback
```
...
| **WHERE** are unified agent resources?        | `.agents/`          |
| **WHAT** can agents access at runtime?        | MCP configuration   |
| **HOW** should agents use reusable prompts?   | `PROMPTS.md`        |
...
│
├── .agents/                      ← Unified agent directory (standard)
│   ├── skills/                   ← AI skills (all tools)
│   │   ├── generate-tests/
│   │   │   ├── SKILL.md
│   │   │   └── scripts/
│   │   ├── create-api-endpoint/
...
│   │   ├── database.md
│   │   └── security.md
│   ├── config.json               ← Unified agent configuration
│   ├── mcp.json                  ← Centralized MCP configuration
│   ├── prompts/                  ← Reusable prompts
│   └── memory/                   ← Persistent agent memory
│       └── project-context.md
│
├── CLAUDE.md                     ← Claude-specific overrides (legacy)
├── .cursor/
│   ├── rules/                    ← Cursor-specific rules (legacy)
│   └── mcp.json                  ← MCP servers for Cursor (legacy)
├── .windsurfrules                ← Windsurf-specific rules (legacy)
├── .github/
│   ├── copilot-instructions.md   ← GitHub Copilot instructions (legacy)
│   └── workflows/                ← CI/CD pipelines
├── .opencode                     ← OpenCode config (legacy)
│
├── docs/
│   ├── api.md
│   ├── build.md
...
- [ ] `.agents/skills/` with at least one skill (start with `generate-tests`)
- [ ] `.agents/config.json` with project configuration
- [ ] `PROMPTS.md` with your most common AI task templates
- [ ] `.agentignore` to protect sensitive files
- [ ] `.agents/mcp.json` if connecting to external services
